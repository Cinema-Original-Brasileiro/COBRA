    ...
    <ul class="atores">
      <li v-for="actor in actorStore.actors" :key="actor.id" class="ator">
        ...
        <div class="informacoes">
          ...
          <h3>Conhecido por</h3>
          <span
            v-for="(movie, index) in actor.movies?.slice(0, 3)"
            :key="movie.id"
            class="actor-movies"
            :style="{ fontSize: actor.movies.slice(0, 3).length > 2 ? '0.7rem' : '0.85rem' }"
          >
            {{ movie.title }}{{ index < actor.movies.slice(0, 3).length - 1 ? ', ' : '' }}
          </span>
          <div v-if="actor.instagramId != undefined">
            <a class="instagram" :href="`https://www.instagram.com/${actor.instagramId}`"
